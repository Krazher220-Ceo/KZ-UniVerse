 ...
 * 
 * Copyright (C) 2024 DataHub ВУЗ-ов РК (KZ UniVerse)
 * 
...
function getMainMenu() {
  return {
    reply_markup: {
      keyboard: [
        [{ text: '🏛️ Все университеты' }, { text: '🏆 Топ-5 вузов' }],
        [{ text: '💻 IT-университеты' }, { text: '💼 Бизнес-программы' }],
        [{ text: '💰 Гранты и стипендии' }, { text: '📊 Сравнить вузы' }],
        [{ text: '📝 Процесс поступления' }, { text: '🌐 Веб-версия' }],
        [{ text: '❓ Помощь' }]
      ],
...
🎓 Я твой персональный AI-помощник по выбору университета в Казахстане.

*Что я умею:*
📚 Информация о 15+ ведущих университетах Казахстана
📊 Умное сравнение вузов по рейтингам, стоимости и программам
💰 Детальная информация о грантах, стипендиях и льготах
🎯 Расчет шансов поступления с учетом ваших баллов
💬 Ответы на любые вопросы об образовании в РК
🔍 Поиск программ по направлениям и городам
📈 Рекомендации на основе ваших интересов

*Используй меню ниже или просто напиши свой вопрос!* 😊
...
bot.on('message', async (msg) => {
  ...
  if (text === '🏛️ Все университеты') {
    let response = '🏛️ *Университеты Казахстана:*\n\n';
    UNIVERSITIES_DB.forEach((uni, index) => {
      ...
      response += `   📍 ${uni.city} | ⭐ ${uni.rating}/5.0\n`;
  ...
  if (text === '🏆 Топ-5 вузов') {
    ...
    let response = '🏆 *Топ-5 университетов Казахстана:*\n\n';
    top5.forEach((uni, index) => {
      response += `${['🥇', '🥈', '🥉', '4️⃣', '5️⃣'][index]} *${uni.shortName}* - ${uni.name}\n`;
      response += `   ⭐ Рейтинг: ${uni.rating}/5.0\n`;
      response += `   📍 ${uni.city}\n`;
      response += `   💰 ${uni.tuition}\n`;
      if (uni.worldRank) response += `   🌍 Мировой рейтинг: ${uni.worldRank}\n`;
      response += '\n';
    });
    await bot.sendMessage(chatId, response, { parse_mode: 'Markdown', ...getMainMenu() });
    return;
  ...
  if (text === '💻 IT-университеты') {
    ...
    let response = '💻 *IT-университеты Казахстана:*\n\n';
    itUnis.forEach((uni, index) => {
      response += `${index + 1}. *${uni.shortName}* - ${uni.name}\n`;
      response += `   📍 ${uni.city} | ⭐ ${uni.rating}/5.0\n`;
      response += `   💰 ${uni.tuition}\n`;
      response += `   🎓 ${uni.programs.filter(p => p.toLowerCase().includes('it') || p.toLowerCase().includes('ai') || p.toLowerCase().includes('software') || p.toLowerCase().includes('data') || p.toLowerCase().includes('computer')).join(', ')}\n\n`;
    });
  ...
  if (text === '💼 Бизнес-программы') {
    const response = `💼 *Лучшие бизнес-программы:*
...
💡 *Совет:* KIMEP - единственный вуз в ЦА с престижной аккредитацией AACSB!`;
  ...
  if (text === '💰 Гранты и стипендии') {
    const response = `🎓 *Гранты и стипендии в Казахстане:*
...
💡 *Совет:* Начните подготовку к ЕНТ заранее!`;
  ...
  if (text === '📊 Сравнить вузы') {
    const response = `📊 *Сравнение университетов:*
...
${UNIVERSITIES_DB.map(u => `• ${u.shortName}`).join('\n')}`;
  ...
  if (text === '📝 Процесс поступления') {
    const response = `📝 *Процесс поступления:*
...
• Зачисление: 25 августа`;
  ...
  if (text === '🌐 Веб-версия') {
    await bot.sendMessage(chatId, `🌐 *Веб-версия KZ UniVerse:*

${API_URL}

На сайте доступно:
• Полная информация о всех университетах
• 3D-туры по кампусам
• Расчет шансов поступления с AI
• Сравнение университетов
• Персональный профиль`, { parse_mode: 'Markdown', ...getMainMenu() });
    return;
  }
  
  if (text === '❓ Помощь') {
    const response = `📚 *Справка KZ UniVerse Bot:*
...
• Новых программах и специальностях
• Важных датах поступления
• Грантах и стипендиях
• Обновлениях рейтингов`;
    await bot.sendMessage(chatId, response, { parse_mode: 'Markdown', ...getMainMenu() });
    return;
  }
  
  // Обработка обычных текстовых сообщений (AI)
  try {
    ...
    await bot.sendMessage(chatId, '❌ Произошла ошибка. Попробуйте переформулировать вопрос или используйте меню.', { ...getMainMenu() });
  }
});
...
async function sendNotification(chatId, type, message) {
  ...
  try {
    await bot.sendMessage(chatId, `${emoji} *Уведомление KZ UniVerse*\n\n${message}`, {
      parse_mode: 'Markdown',
      ...getMainMenu()
    });
  } catch (error) {
...
// Пример: отправка уведомления о важных датах (можно настроить по расписанию)
// setInterval(() => {
